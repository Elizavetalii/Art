...
DROP PROCEDURE IF EXISTS crm_set_order_status(BIGINT, VARCHAR);

DROP FUNCTION IF EXISTS crm_tg_check_ingredient_reservation();
DROP FUNCTION IF EXISTS crm_tg_orderitem_total_recalc();
DROP FUNCTION IF EXISTS crm_tg_prepare_orderitem();
DROP FUNCTION IF EXISTS crm_tg_validate_delivery_date();
DROP FUNCTION IF EXISTS crm_recalculate_order_total(BIGINT);
"""


def apply_postgres_logic(apps, schema_editor):
...
def rollback_postgres_logic(apps, schema_editor):
    if schema_editor.connection.vendor != "postgresql":
        return
    with schema_editor.connection.cursor() as cursor:
...
class Migration(migrations.Migration):
    dependencies = [
        ("crm", "0016_merge_0015_dish_daily_capacity_0015_dish_uom_fields"),
    ]

    operations = [
        migrations.RunPython(
            apply_postgres_logic,
            rollback_postgres_logic,
        ),
    ]
